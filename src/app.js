...
function render() {
  ...
  app.innerHTML = `<main class="app"><header class="topbar"><div class="topbar-title"><h1>poker带入记录本</h1><p class="eyebrow">本机保存</p></div><span class="topbar-divider" aria-hidden="true"></span><div class="total"><p class="eyebrow">全局净带入</p><strong><span class="chip-icon" aria-hidden="true"></span>${formatNumber(global)}</strong></div><button class="clear-records-button" type="button" data-clear-records>清空记录</button></header><nav class="flow-nav"><button data-screen="live" class="${screen === 'live' ? 'active' : ''}">记录</button><button data-screen="settle" class="${screen === 'settle' ? 'active' : ''}">结算</button><button data-screen="ledger" class="${screen === 'ledger' ? 'active' : ''}">账本</button></nav><section class="workspace">${screen === 'live' ? live(state) : screen === 'settle' ? settlement(state) : ledger(state)}</section></main>${recordsModalPlayerId ? recordsDialog(state) : ''}${showResultsModal ? resultsDialog(state) : ''}`;
  bindEvents();
}

function live(state) {
  const recent = state.movements.slice().sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).slice(0, 12);
  return `<section class="flow-page"><section class="players-card"><div class="section-header"><h3>玩家 <span class="player-count">${state.players.length} 人</span></h3><div class="players-tools"><button class="copy-summary-button" type="button" data-copy-summary>${copyStatus || '一键复制'}</button></div></div>${playersTable(state)}${showNewPlayer ? newPlayerEntry() : ''}<p class="error">${esc(message)}</p></section>${state.players.length ? `<section class="timeline"><div class="section-header"><h3>最近记录</h3><button class="text-button" data-screen="ledger">查看全部记录 →</button></div>${recent.length ? recent.map((movement) => timelineItem(state, movement)).join('') : '<p class="muted">暂无记录。</p>'}</section>` : '<section class="empty"><h3>新增玩家后开始记录</h3></section>'}</section>`;
}
...
function quickMovementRow(state, player, index) {
  ...
  return `<tr class="quick-movement-row"><td colspan="5"><form id="quick-movement" class="quick-movement-entry ${theme}"><div class="quick-movement-info"><strong>${index + 1} · ${esc(player.name)} · ${label}</strong></div><label>筹码数量<input name="amount" type="number" min="1" step="1" inputmode="numeric" required autofocus placeholder="请输入数量" /></label><button class="button quick-save-button" type="submit">保存${label}</button><button class="cancel-player-button" type="button" data-close-quick>取消</button></form></td></tr>`;
}
...
function settlementRow(state, player, rate) {
  ...
  return `<div class="settlement-row" data-settlement="${player.id}"><div class="settlement-player"><strong><span class="settlement-player-name">${esc(player.name)}</span><span class="settlement-separator">｜</span><span class="settlement-net-buy-in">${chipValue(summary.netBuyIn)}</span></strong></div><div class="settlement-outcome"><input name="remaining" type="text" inputmode="numeric" value="${settled ? summary.remainingChips : ''}" placeholder="未填写" aria-label="${esc(player.name)}的剩余筹码" /><span class="settlement-separator">｜</span><span class="result"><strong class="${summary.profitLoss >= 0 ? 'positive' : 'negative'}">${settled ? `${summary.profitLoss >= 0 ? '+' : ''}${chipValue(summary.profitLoss)}` : '—'}</strong>${settled && rate ? `<small class="${summary.profitLoss >= 0 ? 'positive' : 'negative'}">${signedMoney(summary.profitLoss, rate)}</small>` : ''}</span></div></div>`;
}
